# Ollama Chat GUI

Десктоп интерфейс за чат с Ollama.

## Нужен софтуер

- **Python 3.8+**
- **Ollama** инсталиран и пуснат ([линк за изтегляне](https://ollama.ai))
- Няколко основни Python пакета (изброени по-долу)

## Бърз старт

1. **Клонирайте или свали това repo**
   ```bash
   git clone <твоето-repo-url>
   cd ollama-chatbot
   ```

2. **Инсталирайте нужните библиотеки**
   ```bash
   pip install PyQt6 requests
   ```

3. **Проверете дали Ollama работи**
   - На Windows/Mac: Просто пусни Ollama приложението
   - На Linux: Изпълни `ollama serve` в терминала

4. **Пусни приложението**
   ```bash
   python main.py
   ```


## Първоначална настройка

При първо отваряне на програмата трябва да се изтегли модел:

1. Натиснете бутона **"Download Model"** в страничното меню
2. Изберете модел
3. Изчакайте да се свали

## Функции

- 💬 Множество чат сесии с автоматично запазване
- 🎨 Тъмна/Светла тема (Ctrl+T за превключване)
- 📥 Разглеждай и свали модели с хубав интерфейс
- 💾 Експортирай чатове като JSON или Markdown
- 🎯 Синтаксис хайлайтинг на код в отговорите
- ⚙️ Настройки за температура и други параметри
- ⌨️ Клавишни комбинации

## Клавишни комбинации

- `Ctrl+N` - Нов чат
- `Ctrl+S` - Запази текущия чат
- `Ctrl+K` - Изчисти чата
- `Ctrl+B` - Покажи/скрий страничното меню
- `Ctrl+T` - Превключи тъмна/светла тема
- `Enter` - Изпрати съобщение
- `Shift+Enter` - Нов ред в съобщението